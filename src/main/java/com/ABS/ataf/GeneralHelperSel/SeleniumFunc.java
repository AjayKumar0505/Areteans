package com.ABS.ataf.GeneralHelperSel;
...
public class SeleniumFunc implements SeleniumFuncLoc {
	...
	/* Anindya */
	public static void clickUsingActionXpath(String xpath) {
		WebElement element = driver.findElement(By.xpath(xpath));
		Actions action = new Actions(driver);
		action.moveToElement(element).click().build().perform();
	}

	public static void clickUsingAction(WebElement element) {
		Actions action = new Actions(driver);
		action.moveToElement(element).click().build().perform();
	}

	/* Anindya mouse hover */
	public static void moveMouseHorizontalRight() {
		Actions action = new Actions(driver);
		action.moveByOffset(30, 0).build().perform();
	}
	...
	public static String goToFrameByTag_NameByWebElement(By element) throws InterruptedException {
		System.out.println("In GoToFrames Method Start");
		...
		for (int i = 0; i < frames.size(); i++) {
			...
			try {
				if (driver.findElement(element).isDisplayed()) {
					// System.out.println("Element" + element.id(frameNameone) + "is present in the
					// frame: " + frameNametwo);
					frameNameone = frameNametwo;
					break;
				}
			} catch (Exception e) {
				System.out.println("Element is not present inside the frame: ");
				driver.switchTo().defaultContent();
			}
		}
		System.out.println("In GoToFrames Method End");
	...
	// *Anindya*
	public static List<WebElement> xpath_GenericMethod_listElements(String xpath) throws Exception {
		driver.switchTo().defaultContent();
		List<WebElement> eles = null;
		System.out.println("in xpath generic for list");
...
//					ele.click();
	...
	/* Abhinav */
	public static void GenericMethod_Click(By webelement) throws Exception {
		try {
			...
			try {
				String framebytagName = goToFrameByTag_NameByWebElement(webelement);

				System.out.println("using getattribute name");
				if (framebytagName == null) {
					goToFrameByTag_NameByWebElement(webelement);
					System.out.println("using getattribute Id");
				}
				WebElement ele1 = driver.findElement(webelement);
				waitForElement(ele1, 1);
				ele1.click();
				waitFor(1);
				// System.out.println("using getattribute Id");
	...
	/*--Anindya--*/
	public static boolean xpath_Genericmethod_VerifyTextEqualsIgnoreCase(String xpath, String expectedText)
			throws Exception {
		boolean flag = false;
		try {
			if (driver.findElement(By.xpath(xpath)).isDisplayed()) {
				flag = verifyTextEqualsIgnoreCase(xpath, expectedText);
				waitFor(1);
...
//		WebDriverWait wait = new WebDriverWait(driver, timeunitForsec);
	...
	public static void waitForElementXpath(String xpath, int timeunitForsec) {
		//ajay
//		WebDriverWait wait = new WebDriverWait(driver, timeunitForsec);
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeunitForsec));
	...
	/*--Anindya --*/
	public static synchronized boolean verifyTextEqualsIgnoreCase(String xpath, String expectedText) {
		boolean flag = false;
		try {
			String actualText = driver.findElement(By.xpath(xpath)).getText().trim();
	...
	public static String getValueFromJSON(String datafile, String key) throws Exception {

		JSONParser parser = new JSONParser();
		String jsonvalue = StringUtils.EMPTY;
		try {
			Object obj = parser.parse(new FileReader("src/test/resources/testdata/" + datafile));
			JSONObject jsonObject = (JSONObject) obj;
			jsonvalue = (String) jsonObject.get(key);
	...
	public Integer xpath_Generic_getTextFromElement(String xpath) {
		int count=0;
		try {
			String countText= driver.findElement(By.xpath(xpath)).getText();
	...
	public static void xpath_MoveToElement(String xpath) throws Exception {
		try {
			if (driver.findElement(By.xpath(xpath)).isDisplayed()) {
				System.out.println("Element" + xpath + "is present outside the frame: ");
				WebElement ele = driver.findElement(By.xpath(xpath));
...
//				System.out.println(s);
				int index = s.indexOf(',');
				String x = s.substring(1, index);
				String y = s.substring(index+1, s.length()-2);
				String position ="("+x +","+y+50+")";
				String script ="return document.elementFromPoint"+position;
	...
	public static void downloadFile(String fileURL, String file) {
...
//	public static String downloadFileFromCloud(String fileURL, String localFileName) throws IOException {
//        URL url = new URL("https://upsc.gov.in/sites/default/files/Notif-CSP-23-engl-010223.pdf");
//        HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
//        BufferedInputStream inputStream = new BufferedInputStream(httpURLConnection.getInputStream());
//        FileOutputStream fileOS = new FileOutputStream("C:\\Images");
// 
...
//	public static void uploadFile(WebDriver driver, String fileInputLocator, String fileURL) throws IOException {
//		String localFileName = System.getProperty("java.io.tmpdir") + "/tempfile";
//		 
//        // Step 1: Download the file from the cloud
//        String downloadedFilePath = downloadFileFromCloud("https://upsc.gov.in/sites/default/files/Notif-CSP-23-engl-010223.pdf", "C:\\Images\\demo.pdf");
// 
//        // Step 2: Locate the file input element and upload the file
//        WebElement fileInputElement = driver.findElement(By.xpath(fileInputLocator)); // Use appropriate selector
//        fileInputElement.sendKeys(downloadedFilePath);
//	}

	
	
//	
//	public static void xpath_MoveToElement(String string) {
